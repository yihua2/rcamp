...
```{r, warning=FALSE, fig.cap=" ", fig.width=12,eval=F}
...
delays <- air[!is.na(air$DepDelay),]
delays[delays$DepDelay<0,]$DepDelay= 0
delays[delays$DepDelay>60,]$DepDelay= 60
p <- ggplot(delays, aes(DepDelay))+geom_histogram() +ggtitle("Flight Delays")
p
```

2) Plot the arrival delay against the departure delay as a scatterplot.
```{r}
delays <- delays[!is.na(delays$ArrDelay),]
ggplot(delays, aes(x = DepDelay, y = ArrDelay))+ geom_point()
...
```{r}
ggplot(delays, aes(x = DepDelay, y = ArrDelay))+geom_point()+ ggtitle("Arrivel delay vs Departure Delay in Minutes") +xlab(label = "Departure Delay") +
  ylab(label = "Arrivial Delay") 
...
```{r}
ggplot(delays, aes(x = factor(DayOfWeek),y=DepDelay))+geom_boxplot()
```
...
delays <- delays[delays$Dest%in%DestSubset,]
ggplot(delays, aes(x = DayOfWeek, y = DepDelay))+geom_boxplot()+facet_wrap(~Dest,nrow = 2, ncol = 5) 
```

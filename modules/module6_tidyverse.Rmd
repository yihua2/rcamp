...
```{r}
df <- gapminder %>% group_by(continent) %>% summarize(median_lifeExp = median(lifeExp))
head(df)
```
...
```{r}
df <- gapminder %>% group_by(continent,year) %>% mutate(pop_cont_year = sum(pop))
head(df)
```
